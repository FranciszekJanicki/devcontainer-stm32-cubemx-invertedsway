...
void MPU6050_AccelerometerAxisStandby(uint8_t XA_Stby, uint8_t YA_Stby, uint8_t ZA_Stby)
{
    ...
    HAL_I2C_Mem_Read(i2c, MPU6050_ADDRESS, MPU6050_RA_PWR_MGMT_2, 1, &tmp, 1, I2C_TIMEOUT);
    tmp &= 0xC7;
    tmp |= ((XA_Stby & 0x1) << MPU6050_PWR2_STBY_XA_BIT) | ((YA_Stby & 0x1) << MPU6050_PWR2_STBY_YA_BIT) |
...
void MPU6050_GyroscopeAxisStandby(uint8_t XG_Stby, uint8_t YG_Stby, uint8_t ZG_Stby)
{
    ...
    HAL_I2C_Mem_Read(i2c, MPU6050_ADDRESS, MPU6050_RA_PWR_MGMT_2, 1, &tmp, 1, I2C_TIMEOUT);
    tmp &= 0xF8;
    tmp |= ((XG_Stby & 0x1) << MPU6050_PWR2_STBY_XG_BIT) | ((YG_Stby & 0x1) << MPU6050_PWR2_STBY_YG_BIT) |
...
{
    i2c = hi2c;
    if (HAL_I2C_IsDeviceReady(i2c, MPU6050_ADDRESS, 10, 100) != HAL_OK) {
        printf("DEVICE ISNT READY BRUH\n\r");
    } else {
        printf("DEVICE IS READY BRUH\n\r");
    }

    uint8_t tmp;
    HAL_I2C_Mem_Read(i2c, MPU6050_ADDRESS, MPU6050_RA_WHO_AM_I, 1, &tmp, 1, I2C_TIMEOUT);
    printf("WHOM_AM_I: %d", tmp << 1);
